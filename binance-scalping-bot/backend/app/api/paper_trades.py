...
class PaperTradeAPI:
    ...
    def _map_trade(cls, row: dict, btc_following: bool | None = None) -> PaperTrade:
        ...
        return PaperTrade(
            ...
            can_enter=cls._coerce_optional_bool(row.get("can_enter")),
            blocked_reason=str(row["blocked_reason"]) if row.get("blocked_reason") is not None else None,
            entry_logic_flag=str(row["entry_logic_flag"]) if row.get("entry_logic_flag") is not None else None,
        ...
        trade_id = repo.create_open_trade(
            {
                ...
                "entry_logic_flag": "MANUAL_MARKET",
